...
from data_processing import creat_dataloader_val,CustomDataset, get_val_datasetlist
...
def main():
    ...
    train_model_num = 1    #training epoch
    
    flag1=1

    folder = input_path

    Folder_Name = "Independent_Validation" 
    ...
    train_df_list,test_df_list = get_val_datasetlist(input_path)
    
    for filedata1,filedata2 in zip(train_df_list,test_df_list):
        train_loader,test_loader,all_loader, original_y_train,original_y_test,original_p_train,original_p_test,original_id_train,original_id_test = creat_dataloader_val(filedata1,filedata2)
    ...
    train_model(model, criterion, optimizer, train_loader, num_epochs=train_model_num)  
    
    #--------------------------------------------------------# Resampling and Modality Integration     
    
    dataset_test = CustomDataset(TEST_data)
    test_loader = DataLoader(dataset_test, batch_size=10, shuffle=True, drop_last=True)  
    ...
    # Total of 13 modalities, we only consider 12-element combinations
        ...
        for L in range (0,13):
            ...
            for perm in selected_permutations:
                if( f'{L}' not in perm):
                    ...
                    data_t1,verify= get_dataset_TRAIN_data_subset(TRAIN_data_subset,string_perm,index)
    ...
    for h in range(d):
        for m, value in enumerate(position_lowCon):
            ...
            # A modality is deemed low-contribution if ≥50% samples identify it as such
